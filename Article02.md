...
|No.|ビュー|説明|おもなモデル|
|--|--|--|--|
|1|ドメインビュー|境界図けられたコンテキストをシステム上に実現するための設計を行う。|境界付けられたコンテキスト、コンテキストマップ|
...
アーキテクチャ設計書のドメインビューでは、境界付けられたコンテキストの設計自体は行わず、業務の分析・設計文書にて事前に定義されているものとして、その実現を設計します。

コンテキスト間の関係には、腐敗防止層や共有カーネルといった関係の種別があります。

ドメインビューでは、関係別の設計・実装方針を決定します。
...
非機能全体の定義は、ユースケースビューと同様に、非機能要件定義書のような文書で定義されている前提とします。非機能要件には、たとえば保守・運用に関する要件などが含まれます。

非機能要件ビューでは、定義ずみの非機能要件からアーキテクチャ上考慮が必要な要件を特定し、その実現方法を設計します。
...
MVVMパターンで設計する場合、ViewはViewModelに依存しますが、その逆をしてしまうと循環参照になってしまいます。しかし細かな実装をしていると、ついついViewModelからViewを操作してしまいたくなります。それで正しく動作することもありますが、WPFの場合はListなどが仮想化されている関係上、Viewを直接操作してしまうと不具合のもとになることもあります。また単純に依存関係が双方向になると、コードが負いきれなくなって、後日メンテナンス困難なスパゲッティなコードになりがちです。

そのため依存関係を適切に制御するため、プロジェクトを動分割するかは、非常に重要な設計になります。
...
しかし実際にアーキテクチャ設計を行っていく場合、ユースケースビューと論理ビュー、非機能要件ビューと論理ビューは、頻繁に行ったり来たりしながら記述します。どちらかを先に完璧に書きあげるという訳には行きません。

また論理ビューから実装ビュー・配置ビューは概ねその方向に流れて設計しますが、論理ビュー自体がユースケースビューや非機能要件ビューの設計に伴い頻繁に更新されるため、ウォーターフォール的な流れにはならず、インクリメンタルなプロセスになります。
...
これはいい加減で良いという意味ではありません。とくに類似のアーキテクチャに対する経験が多い方は、この段階でかなり正確な設計が可能です。ただ、悩んで何日も手が止まってしまうくらいなら、先に進めてからフィードバックしてくれば良いと思います。

なお前編で設計するビューには実装ビューが含まれません。実装ビューは論理ビューで設計された代表的なオブジェクトを、どのコンポーネントに含めるか設計するビューです。

初期論理ビューではレイヤー構成の仮決定だけして、オブジェクトの抽出は、後編の代表的なユースケースを実現しながら行います。実装ビューを設計できるほどオブジェクトが登場しないため、前編では実装ビューの設計を省略します。

## 後編

後編ではユースケースや非機能の実現を設計します。

その中で、論理ビューや実装ビューにフィードバックしていき、アーキテクチャ全体の精度を上げていきます。
